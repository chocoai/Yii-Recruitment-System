...
return array (
    ...
    'An operation is a permission to perform a single operation, for example accessing a certain controller action.' => '一个操作权限可以执行一个操作，例如访问一个特定的 controller action。',
    'Operations exist below tasks in the authorization hierarchy and can therefore only inherit from other operations.' => '以下的操作存在授权等级的任务中，因此只能从其他操作中继承。',
    'Here you can view which permissions has been assigned to each user.' => '在这里，您可以查看用户已被指派了哪些角色。',

    'A task is a permission to perform multiple operations, for example accessing a group of controller action.' => '一个任务可以执行多个操作，例如访问一组 controller action。',
    'Tasks exist below roles in the authorization hierarchy and can therefore only inherit from other tasks and/or operations.' => '以下的任务存在授权等级中的角色中，因此只能从其他操作/任务中继承。',

    'A role is group of permissions to perform a variety of tasks and operations, for example the authenticated user.' => '一个角色是一组权限可以执行各种各样的任务和操作，例如验证用户的身份。',
    'Roles exist at the top of the authorization hierarchy and can therefore inherit from other roles, tasks and/or operations.' => '以下角色存在授权等级的最顶级，因此可以从其他角色/任务/或操作中继承。',

    'Here you can view and manage the permissions assigned to each role.' => '在这里你可以查看和管理分配给每个角色的权限。',
    'Authorization items can be managed under {roleLink}, {taskLink} and {operationLink}.' => '授权项目可以根据{roleLink}，{taskLink}和{operationLink}来管理。',
    'Generate items for controller actions' => '为controller actions生成项目',
